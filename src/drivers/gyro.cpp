...
i32 bmiDataRaw[6] = {0, 0, 0, 0, 0, 0};
i32 *gyroDataRaw;
i32 *accelDataRaw;

#ifdef ENABLE_GYRO_BB
i16 gyroBB[2000][6] = {0};
fix32 vAccelBB[2000] = {0};
fix32 vAccelSlowBB[2000] = {0};
u16 gyroBBFrame = 0;
bool gyroBBActive = false;
#endif
...
void gyroLoop() {
	...
	if (gyroBBActive) {
		if (gyroBBFrame < 2000) {
			gyroBB[gyroBBFrame][0] = accelDataRaw[0];
			gyroBB[gyroBBFrame][1] = accelDataRaw[1];
			gyroBB[gyroBBFrame][2] = accelDataRaw[2];
			gyroBB[gyroBBFrame][3] = gyroDataRaw[0];
			gyroBB[gyroBBFrame][4] = gyroDataRaw[1];
			gyroBB[gyroBBFrame][5] = gyroDataRaw[2];
			vAccelBB[gyroBBFrame] = vAccel;
			vAccelSlowBB[gyroBBFrame] = vAccelSlow;
		} else if (gyroBBFrame == 2000) {
			makeSound(500, 100);
		}
		gyroBBFrame++;
		if (gyroBBFrame == 10000) {
			gyroBBActive = false;
			makeSound(2000, 100);
		}
	} else if (gyroDataRaw[0] >= 3200 || gyroDataRaw[0] <= -3200 || gyroDataRaw[1] >= 3200 || gyroDataRaw[1] <= -3200 || gyroDataRaw[2] >= 3200 || gyroDataRaw[2] <= -3200) {
		gyroBBFrame++;
		if (gyroBBFrame == 20) {
			gyroBBFrame = 0;
			gyroBBActive = true;
		...
		if (Serial.available()) {
			...
			for (int i = 0; i < 2000; i++) {
				Serial.printf("%d;%d;%d;%d;%d;%d;%.2f;%.2f\n", gyroBB[i][0], gyroBB[i][1], gyroBB[i][2], gyroBB[i][3], gyroBB[i][4], gyroBB[i][5], vAccelBB[i].getf32(), vAccelSlowBB[i].getf32());
			}
		}
	}
#endif
...
int gyroInit() {
	...
	gpio_init(PIN_GYRO_INT);
	gpio_set_dir(PIN_GYRO_INT, GPIO_IN);
	...
	if ((data & 0x6F) != 0x01) {
		sleep_ms(5000);
		Serial.printf("Failed to load BMI270, wrong initialization status: 0x%02X\n", data); // initialization status should be 0x01
		return 2;
	}
	...
	data = 1 << 7 | 1 << 6 | 0x00 << 4 | 0x0C; // performance optimized, 1600Hz
	...
	// significant motion detection for standby mode
	// // FEAT_PAGE: page (2...0)
	// data = 0x02;
	// regWrite(SPI_GYRO, PIN_GYRO_CS, (u8)GyroReg::FEAT_PAGE, &data, 1, 500);
	// // SIGMO_1: block_size (15...0)
	// u16 data2 = 0x0002;
	// regWrite(SPI_GYRO, PIN_GYRO_CS, (u8)GyroReg::SIGMO_1, (u8 *)&data2, 2, 500);
	// // SIGMO_2: out_conf (4...1), enable (0)
	// data2 = 0x0003;
	// regWrite(SPI_GYRO, PIN_GYRO_CS, (u8)GyroReg::SIGMO_2, (u8 *)&data2, 2, 500);

	// any motion detection for standby mode
	// FEAT_PAGE: page (2...0)
	data = 0x01;
	regWrite(SPI_GYRO, PIN_GYRO_CS, (u8)GyroReg::FEAT_PAGE, &data, 1, 500);
	// ANYMO_1: select_z (15), select_y (14), select_x (13), block_size (12...0)
	u16 data2 = 0xE005;
	regWrite(SPI_GYRO, PIN_GYRO_CS, (u8)GyroReg::ANYMO_1, (u8 *)&data2, 2, 500);
	// ANYMO_2: enable (15), out_conf (14...11), threshold (10...0)
	data2 = 0xB820;
	regWrite(SPI_GYRO, PIN_GYRO_CS, (u8)GyroReg::ANYMO_2, (u8 *)&data2, 2, 500);

	// INT1_MAP_FEAT: any_motion_out(6), no_motion_out(5), wrist_gesture_out(4), wrist_wear_wakeup_out(3), activity_out(2), step_counter_out(1), sig_motion_out(0)
	data = 0b01000001;
	regWrite(SPI_GYRO, PIN_GYRO_CS, (u8)GyroReg::INT1_MAP_FEAT, &data, 1, 500);
...
void gyroGetData(i32 *buf) {
	i16 readBuf[6];
	regRead(SPI_GYRO, PIN_GYRO_CS, (u8)GyroReg::ACC_X_LSB, (u8 *)readBuf, 12);
	buf[0] = readBuf[2];
	buf[1] = -readBuf[1];
	buf[2] = readBuf[0];
	buf[3] = readBuf[5];
	...
	buf[0] -= (buf[4] * buf[4] + buf[5] * buf[5]) / 600000; // centrifugal acceleration
	buf[1] -= (buf[5] * buf[5] + buf[3] * buf[3]) / 55000;
	buf[2] += (buf[3] * buf[3] + buf[4] * buf[4]) / 400000;
